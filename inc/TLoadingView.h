...
//+ (TLoadingView *)getInstance;
+ (void)didActive;

/// 加载Loading cancelDel:取消委托 text:文字
+ (void)showLoadingWithDelegate:(id <CancelRequestDelegate>)cancelDel text:(NSString *)text;
/// 加载Loading cancelDel:取消委托
+ (void)showLoadingWithDelegate:(id <CancelRequestDelegate>)cancelDel;
